...
function _initModelPickerDropdown() {
  ...
  function _populate(filter) {
    ...
    function _addRow(m) {
      ...
      const _epDisplay = m.epName && m.display && !m.display.toLowerCase().includes(m.epName.toLowerCase().split('/').pop()) ? m.epName : '';
    ...
    if (all.length > BROWSE_ALL_LIMIT) {
      ...
      // Show Category > Tier > Location groups with collapsible sections for all models.
      const rest = all.filter(m => !shown.has(m.mid));
      
      function getUseCategory(m) {
        const id = (m.mid || '').toString().toLowerCase();
        if (id.includes('embed') || id.includes('bge') || id.includes('nomic')) return 'Embeddings';
        if (id.includes('coder') || id.includes('code') || id.includes('starcoder')) return 'Code';
        if (id.includes('vision') || id.includes('llava') || id.includes('vl')) return 'Vision';
        return 'Chat';
      }
      
      function getPowerTier(m, loc) {
        const id = (m.mid || '').toString().toLowerCase();
        
        if (loc === 'Remote') {
          if (/(gpt-4|opus|sonnet|pro|large)/.test(id)) return 'High Power';
          if (/(gpt-3\.5|haiku|flash|mini|small)/.test(id)) return 'Low Power';
          return 'Medium Power';
        }
        
        // Baseado no hardware local
        const paramMatch = id.match(/(\d+(?:\.\d+)?)[bx]/); 
        if (paramMatch) {
          const size = parseFloat(paramMatch[1]);
          if (size > 32) return 'Amdy Extreme Hardware'; // Exige mais que a RX 9060 XT
          if (size <= 8) return 'Tell Hardware';         // Roda na GTX 1060
          return 'Amdy Hardware';                        // Roda na RX 9060 XT
        }
        
        return 'Amdy Hardware';
      }
      
      function getLocation(m) {
        const p = (m.providerText || '').toLowerCase();
        const url = (m.url || '').toLowerCase();
        
        if (url.includes('localhost') || url.includes('127.0.0.1')) return 'Amdy';
        if (url.includes('192.168.') || url.includes('10.0.') || p.includes('tell')) return 'Tell';
        if (p.includes('local')) return 'Amdy';
      ...
      rest.forEach(m => {
        const cat = getUseCategory(m);
        const loc = getLocation(m);
        const tier = getPowerTier(m, loc);
        const groupKey = `${cat} — ${tier} — ${loc}`;
        if (!groups.has(groupKey)) groups.set(groupKey, []);
        groups.get(groupKey).push(m);
      });
      
      const catOrder = { 'Chat': 1, 'Code': 2, 'Vision': 3, 'Embeddings': 4 };
      const tierOrder = { 
        'High Power': 1, 'Medium Power': 2, 'Low Power': 3,
        'Amdy Extreme Hardware': 1, 'Amdy Hardware': 2, 'Tell Hardware': 3
      };
      const locOrder = { 'Amdy': 1, 'Tell': 2, 'Remote': 3 };

      const sorted = [...groups.keys()].sort((a, b) => {
        const [catA, tierA, locA] = a.split(' — ');
        const [catB, tierB, locB] = b.split(' — ');
        
        const c1 = catOrder[catA] || 99, c2 = catOrder[catB] || 99;
        if (c1 !== c2) return c1 - c2;
        
        const t1 = tierOrder[tierA] || 99, t2 = tierOrder[tierB] || 99;
        if (t1 !== t2) return t1 - t2;

        const l1 = locOrder[locA] || 99, l2 = locOrder[locB] || 99;
        if (l1 !== l2) return l1 - l2;
        
        return a.localeCompare(b);
      });

      sorted.forEach(groupKey => {
        const models = groups.get(groupKey);
        // sort by display name safely
        models.sort((m1, m2) => {
          const d1 = m1.display || '';
          const d2 = m2.display || '';
          return d1.localeCompare(d2);
        });
        const isCollapsed = _collapsedProviders.has(groupKey);
        ...
        header.innerHTML =
          ...
          + `<span class="mp-provider-name">${groupKey}</span>`
          + `<span class="mp-provider-count">${models.length}</span>`;
        header.addEventListener('click', (e) => {
          e.stopPropagation();
          if (_collapsedProviders.has(groupKey)) {
            _collapsedProviders.delete(groupKey);
            _justExpandedProvider = groupKey;
          } else {
            _collapsedProviders.add(groupKey);
        ...
        if (!isCollapsed) {
          ...
          group.className = 'mp-provider-group' + (_justExpandedProvider === groupKey ? ' mp-just-expanded' : '');
          models.forEach(m => {
            _addRow(m);
          ...
          if (_justExpandedProvider === groupKey) _justExpandedProvider = null;
        }
      });
